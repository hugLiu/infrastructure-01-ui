<template>
    <div :id="_id" v-show="!hidden">
        <div v-if="!!_data">
            <table class="table table-bordered table-hover vue-table template-table vue-table-layout">
                <thead>
                    <tr>
                        ...
                        <th v-for="(f, index) of _data.fields" class="vue-table-th vue-table-cell-field" @click="sort(index, $event)" v-if="f.visible" :style="{ width: f.size?f.size:'100%' }">
                            <div v-html="f.title" :title="f.title" class="vue-table-th-content text-ellipsis inline-block"></div>
                            <div class="th-icon">
                                <div v-if="f.sortable" class="inline-block">
                                ...
                                <dropdowntemplate :itemlist="_data.fields"></dropdowntemplate>
                            </div>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="n in _pageSize" v-if="(_pageIndex - 1) * _pageSize + n <= _count" @click.stop="clickRow(n, $event)">
                        ...
                        <td v-for="f of _data.fields" class="vue-table-cell-record" v-if="f.visible">
                            <div v-if="f.operationTemplate">
                                <strparsetemplate :strtemplate="f.operationTemplate" :keys="_data.fields" :values="_data.records[n-1]"></strparsetemplate>
                            </div>
                            <div v-else :title="_data.records[n-1][f.name]" class="text-ellipsis">{{_data.records[n-1][f.name]}}</div>
                        </td>
                    </tr>
                </tbody>
                <tfoot>
                    <tr>
                        <td :colspan="_footLength" class="vue-table-cell-pager">
                            ...
                            <div class="vue-table-pager" v-if="_count > _pageSize">
                                <page :pagesize="_pageSize" :count="_count" :items="5" :onselectitem="selectPage" ref="page"></page>
                            </div>
                        </td>
                    </tr>
                </tfoot>
    ...
    export default {
        ...
        methods: {
            sort(fieldIndex, event) {
                if (event.target.className == "icon-reorder") {
                    // dropdown event not sort event
                    return
                }
                this._pageIndex = 1
                ...
                if (this.onsort) {
                    if (this.id) {
                        let returnRecords = this.onsort(
                            soModels.getReturnModel(this._id, $detail) // invoke common function return additional info
                        )
                    ...
                    if (tempRecords) {
                        tempRecords = this.sortHelper(
                            JSON.parse(tempRecords),
                            sortField,
                            ascendingOrder
                        )
            ...
            selectPage(pageIndex) {
                ...
                if (this.onselectpage) {
                    if (this.id) {
                        let returnRecords = this.onselectpage(
                            soModels.getReturnModel(this._id, $detail)
                        )
                        returnRecords && this.updateRecords(returnRecords)
                    } else {
